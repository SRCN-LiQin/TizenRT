...
ui_widget_t g_clock_text;
void update_araui_text(char* str)
{
	ui_text_widget_set_text(g_clock_text, str);
}
...
static void view_create_cb(ui_view_t view)
{
	...
	g_pages[0] = ui_widget_create(CONFIG_UI_DISPLAY_WIDTH, CONFIG_UI_DISPLAY_HEIGHT);
	g_clock_text = ui_text_widget_create(CONFIG_UI_DISPLAY_WIDTH, 100, g_default_font_20, "");
	ui_text_widget_set_align(g_clock_text, UI_ALIGN_CENTER | UI_ALIGN_MIDDLE);
	ui_widget_add_child(g_pages[0], g_clock_text, 0, 60);
	ui_text_widget_set_text(g_clock_text, "LISTENING...");
...
#endif
{
	if (test_first) {
		...
		if (argc > 1) {
			ui_text_widget_set_text(g_clock_text, argv[1]);
		} else {
			printf("input the text you want to display.\n");
		}
	}
//	ui_stop();
