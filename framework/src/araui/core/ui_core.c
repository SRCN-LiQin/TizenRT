...
static void _ui_redraw(uint32_t dt)
{
	...
	while (redraw_area) {
		...
		if (view) {
			...
			ui_hal_redraw(redraw_area->x, redraw_area->y, redraw_area->width, redraw_area->height);
...
static void *_ui_core_thread_loop(void *param)
{
	...
	uint32_t test_dt;
	struct timespec test_before;
	clock_gettime(CLOCK_MONOTONIC, &test_before);
	...
	while (g_core.state == UI_CORE_STATE_RUNNING) {
		...
		test_dt = ((now.tv_sec - test_before.tv_sec) * 1000) + ((now.tv_nsec - test_before.tv_nsec) / 1000000);
		test_before = now;
		//printf("core1 dt %d\n",test_dt);
