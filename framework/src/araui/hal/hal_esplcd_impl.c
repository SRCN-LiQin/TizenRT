...
#if DEBUG_BUTTON_SIM
static iotbus_gpio_context_h g_button;
static pid_t g_timerpid;
static struct timespec g_prev_click;
static struct timespec g_prev_rel;
static int g_pressed;
static int g_longpressing;
static int g_checkclick;

static int longpress_check_task(int argc, char *argv[])
{
	uint32_t dt;
	struct timespec now;
	...
	while(1) {
		...
		if (g_pressed) {
			...
			dt = ((now.tv_sec - g_prev_click.tv_sec) * 1000) + ((now.tv_nsec - g_prev_click.tv_nsec) / 1000000);
			if (dt > 400) {
				// Send Long press event here!
				printf("Long pressing!\n");
				g_longpressing = 1;
			}
		} else if (g_checkclick) {
			clock_gettime(CLOCK_MONOTONIC, &now);
			dt = ((now.tv_sec - g_prev_rel.tv_sec) * 1000) + ((now.tv_nsec - g_prev_rel.tv_nsec) / 1000000);
			if (dt > 500) {
				printf("Click!\n");
				//Send Click Event here!
...
static void gpio_callback_event(void *user_data)
{
    ...
    int value = iotbus_gpio_read(g_button);
    ...
    if (value) {
		...
		clock_gettime(CLOCK_MONOTONIC, &g_prev_rel);
    } else {
		g_pressed = 1;
		clock_gettime(CLOCK_MONOTONIC, &now);
		dt = ((now.tv_sec - g_prev_click.tv_sec) * 1000) + ((now.tv_nsec - g_prev_click.tv_nsec) / 1000000);
		if (dt < 1000) {
			printf("Double Click!\n");
			//Send Double Click event here!

			g_longpressing = 1; //to avoid click event for the following release
		}
		g_prev_click = now;
    }
}
...
#if DEBUG_BUTTON_SIM
	iotapi_initialize();
	g_button = iotbus_gpio_open(2);
	iotbus_gpio_register_cb(g_button, IOTBUS_GPIO_EDGE_BOTH, gpio_callback_event, NULL);
	g_timerpid = task_create("button_sim", RENDER_TASK_PRIO, 0x1000, longpress_check_task, NULL);
#endif
...
ui_error_t ui_hal_deinit(void)
{
	...
	task_delete(g_tpid);
#if DEBUG_BUTTON_SIM
	iotbus_gpio_close(g_button);
	task_delete(g_timerpid);
#endif
...
void ui_hal_draw_pixel(int32_t x, int32_t y, ui_color_t color)
{
	uint32_t alpha;
	uint32_t bg;
	uint32_t fg;
...
void ui_hal_draw_image(int32_t x, int32_t y, ui_asset_t image)
{
	...
	if (!ui_image_asset_has_alpha(body->pixel_format)) {
		printf("Sorry, now only support an image with alpha values!\n");
	}
